...
const SideBarItem = ({ href, title, children }: ISidebarItem) => {
  ...
  if (children && children.length > 0) {
    return (
      <li className="m-2">
        <button
          type="button"
          className="flex items-center w-full p-2 text-grey-200 transition duration-75 rounded-lg group hover:bg-amber-100"
  ...
  return (
    <li className="m-2" key={title}>
      <Link href={href}>
        <div
          className={`flex items-center w-full p-2 transition duration-75 rounded-lg group hover:bg-amber-100`}
        >
